...
const previewImg = function (params) {
  ...
  const maxZindex =
    window.getComputedStyle(document.body).zIndex === "auto"
      ? 0
      : window.getComputedStyle(document.body).zIndex;
  document.getElementsByClassName("preview-img-zwj__mask")[0].style.zIndex =
    maxZindex + 1;
  document.getElementsByClassName("preview-img-zwj__content")[0].style.zIndex =
    maxZindex + 2;
  document.getElementsByClassName("preview-img-zwj__action")[0].style.zIndex =
    maxZindex + 3;
...
previewImg.prototype = {
  resetStyle: function () {
    const comStyle = window.getComputedStyle(
      document.getElementById("preview-img-zwj__img")
    );
  ...
  _renderEvent() {
    document.querySelector(".preview-img-zwj__content .close").onclick =
      this._closeFun;
    document.querySelector(
      ".preview-img-zwj__content .preview-img-zwj__action .fangda"
    ).onclick = this._fangdaFun;
    document.querySelector(
      ".preview-img-zwj__content .preview-img-zwj__action .reduce"
    ).onclick = this._reduceFun;
    document.querySelector(
      ".preview-img-zwj__content .preview-img-zwj__action .rotate-left"
    ).onclick = this._rotateLeft;
    document.querySelector(
      ".preview-img-zwj__content .preview-img-zwj__action .rotate-right"
  ...
  _rotate: function (type) {
    ...
    const deg =
      current &&
      +dom.style.transform.substring(7, dom.style.transform.length - 4);
    const currentDeg = type ? deg + 90 : deg - 90;
    dom.style.transform = current
